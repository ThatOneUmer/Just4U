...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$errors) {
        if (db_user_exists($email, $username)) {
            ...
            $res = db_create_user($username, $email, $password, $role);
            if ($res['ok']) {
                // Load created user
                ...
                if ($user) {
                    if ($user['role'] === 'seller') {
                        ...
                        $defaultStore = $user['username'] . ' Store';
                        db_create_seller((int)$user['id'], $defaultStore);
                        // Do NOT auto-login sellers; redirect to login with pending notice
                        // Ensure any existing session auth is cleared
                        unset($_SESSION['user_id'], $_SESSION['username'], $_SESSION['role']);
                        $msg = urlencode('Your seller account is pending approval. Please wait for an administrator to approve your account.');
                        header('Location: login.php?notice=' . $msg);
                        exit;
                    } else {
                ...
                $errors[] = 'Failed to create account. ' . ($res['error'] ? htmlspecialchars($res['error']) : 'Please try again later.');
            }
        }
    }
}
...
<head>
    ...
    <title>Signup | Just4U Gaming</title>
...
<body>
    ...
    <section class="support-hero" style="margin-top:100px;">
        <div class="container">
            <div class="support-hero-content">
                ...
                <p class="page-subtitle">Join Just4U Gaming</p>
            </div>
        </div>
    </section>
    ...
    <section class="support-content">
        <div class="container">
            <div class="support-layout" style="display:flex; justify-content:center;">
                <main class="support-main" style="max-width:520px; width:100%;">
                    <?php if ($errors): ?>
                        <div class="notification notification-error" style="margin-bottom:16px; background:#FF6B6B; color:#fff; padding:12px 16px; border-radius:12px;">
                            <i class="fas fa-exclamation-circle"></i> <?php echo htmlspecialchars(implode(' ', $errors)); ?>
                        </div>
                    <?php endif; ?>
                    <form method="post" class="support-form" style="background: var(--secondary-bg); padding:24px; border:1px solid rgba(0,255,136,0.2); border-radius:16px;">
                        ...
                        <p style="margin-top:12px; text-align:center; color:var(--text-secondary);">Already have an account? <a href="login.php">Login</a></p>
                    </form>
                </main>
            </div>
        </div>
    ...
    <script src="script.js?v=auth2"></script>
</body>
</html>
